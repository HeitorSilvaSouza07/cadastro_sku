...
def trocar_mil_por_miu(sku):
    partes = sku.split("-")
    partes = ["HEXAGONAL" if p == "HEXAGONLA" else p for p in partes]
    return "-".join(partes)
    

df = pd.read_excel(ARQUIVO_ENTRADA)

df_miu = df[df["SKU"].str.contains(r"(?:^|-)HEXAGONAL(?:-|$)", regex=True, na=False)].copy()
df_mil = df[df["SKU"].str.contains(r"(?:^|-)HEXAGONLA(?:-|$)", regex=True, na=False)].copy()
...
print(f"Total HEXAGONAL extraídos: {len(df_resultado)}")
print("Finalizado!")
